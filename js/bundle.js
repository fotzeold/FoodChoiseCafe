...
function cards() {
  // --------------- Class - card -------------- //
  class cardCreate {
    constructor(img, title, text, price, selector) {
      this.img = img;
      this.title = title;
      this.text = text;
      this.price = price;
      ...
      this.parent = document.querySelector(selector), this.classes = classes;
    }
...
function modal() {
  ...
  const modalButtonsTrigger = document.querySelectorAll('[data-modal]'),
        ...
        modalWindow = document.querySelector('.modal'); // modalWindowTimer ######

  let modalWindowTimer = setTimeout(() => {
    if (!modalWindow.classList.contains('show')) {
      hiddenToggle();
    }
  }, 3000); // modalWindowButtonToggler ######
  ...
  window.addEventListener('scroll', modalScrollEndShow); // modalWindowFunction ######
...
function slider() {
  ...
  function slideMoveRight() {
    if (positionX === widthWrapper * (slides.length - 1)) {
      positionX = -widthWrapper;
    }
